...
HAVING COUNT(MALOP) >= ALL (SELECT COUNT(MALOP)
							FROM GIANGDAY GD1, GIAOVIEN GV1
							WHERE GD1.MAGV = GV1.MAGV AND GD1.HOCKY = GD.HOCKY AND GD1.NAM = GD.NAM
							GROUP BY GD1.HOCKY, GD1.NAM, GV1.MAGV)
...
ON A.MAHV = HV.MAHV
-- 34.	* Tìm học viên (mã học viên, họ tên) đã thi tất cả các môn
--và đều đạt (chỉ xét lần thi sau cùng).
